<?php /*a:1:{s:87:"D:\soft\environment\InstallDir\htdocs\tp51\application\api\view\v1\home\user\lists.html";i:1571832131;}*/ ?>
<!doctype html>
<html>
<head>
    <meta charset="utf-8">
    ...
    ws.onmessage=function(e){
        ...
        switch (message.type){
            case  "init":
                ...
                var bild = '{"type":"bind","fromid":"'+message.client_id+'"}';
                ws.send(bild);
                list();
                return;
            case "text":
                ...
                function(res){
                    ...
                    $.each(res,function(index,content){
                        if(content.countNoread==0){
                            $(".chat-fui-content").append('<div onclick=redi("'+content.chat_page+'") class="chat-list flex" ><div class="chat-img"  style="background-image: url('+content.head_url+')"></div> <div class="chat-info"> <p class="chat-merch flex"> <span class="title t-28">'+content.username+'</span> <span class="time">'+mydate(content.last_message.time)+'</span> </p> <p class="chat-text singleflow-ellipsis">'+content.last_message.message+'</p> </div></div>');
